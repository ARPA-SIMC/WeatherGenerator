...
int main(int argc, char *argv[])
{
    ...
    for (int i=0;i<n;i++)
    {
        x[i]= (float*)calloc(nrPredictors, sizeof(float));
    }
    x[0][0]= 2;
    x[1][0]= 3;
    x[2][0]= 5;
    ...
    y[3]=5;
    y[4]=8;
    statistics::weightedMultiRegressionLinear(x,y,y,n,&q,m,nrPredictors);
    //statistics::multiRegressionLinear(x,y,n,&q,m,nrPredictors);
    //double detResult = 0.3;
    //detResult = matricial::determinant(x,nrPredictors);
    //matricial::linearSystemResolutionByCramerMethod(y,x,n,m);
    float funzione;
    for (int i=0;i<n;i++)
    {
        for (int j=0;j<n;j++)
        {
            funzione = m[0]*i+m[1]*j+q;
            printf("%d %d %f\n",i,j,funzione);
        }
    }
    printf("%f %f %f\n",m[0],m[1],q);
    ...
    std::vector <std::vector <float>> x(n,std::vector <float>(nrPredictors));
...
void printSimulationResults(ToutputWeatherData* output,int nrStations,int lengthArray)
{
    ...
    for (int iStation=0; iStation<nrStations;iStation++)
    {
        ...
        for (int m=0; m<lengthArray; m++)
        {
            ...
            {
                stream <<  output[iStation].daySimulated[m] << "/" << output[iStation].monthSimulated[m] << "/"
                      << output[iStation].yearSimulated[m] << "," << output[iStation].doySimulated[m] << ","
                      << output[iStation].minT[m]<< "," << output[iStation].maxT[m]<< "," << output[iStation].precipitation[m]
                      << "," << output[iStation].minTClimate[m]<< "," << output[iStation].maxTClimate[m] << "\n";
